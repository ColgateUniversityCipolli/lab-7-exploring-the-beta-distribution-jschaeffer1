...
library(xtable)
distrib.table <- xtable(distribution_table, label="distrib.tab", caption="Properties in comparison to differing parameter values")
@
...
<<size='scriptsize', echo=FALSE, results=FALSE, message = FALSE, warning = FALSE>>=
library(xtable)
charac.estim <- xtable(sample_summaries, label="estim.tab", caption="Estimated characteristics of the beta distribution for specified parameters")
@
...
\begin{figure}[H]
 \begin{center}
 \includegraphics[scale=0.7]{estimator_comparison.pdf}
 ...
 \begin{figure}[H]
 \begin{center}
 \includegraphics[scale=0.45]{estimate_density.pdf}
